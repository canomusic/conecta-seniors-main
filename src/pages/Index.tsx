...
export default function Index() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // --- SISTEMA DE REINICIO DE TAREAS A LAS 00:00 ---
    ...
    if (lastReset !== todayStr) {
      ...
      if (allTasks) {
        const dailyToReset = allTasks.filter(t => t.title.includes("(Todos los días)") && t.completed);
  ...
  const addTask = async (title: string, time: string, isDaily: boolean) => {
    if(!title) return;
    const finalTitle = isDaily ? `${title} (Todos los días)` : title;
  ...
  return (
    <div className="h-full flex flex-col bg-[#F8F9FA]">
      ...
      <div className="p-4 space-y-3 overflow-y-auto mt-4">
        {/* BOTÓN ROJO ESTILO AGENDA FAMILIAR */}
        <button 
        ...
        >
          <div className="text-left flex flex-col">
            <p className="font-black text-3xl text-white leading-none uppercase">112</p>
            <p className="text-white/80 font-bold text-sm mt-2">Llamar a emergencias</p>
          </div>
        ...
        <p className="text-center text-slate-400 text-xs font-bold mt-6 px-4">Pulsa la caja roja para iniciar una llamada de teléfono real.</p>
      </div>
    </div>
  );
}
...
function Login({ onAccess }: any) {
  ...
  const handleF = async (m: any) => {
    ...
    if (error) return toast.error("Error al entrar");
    ...
    if (m === 'reg') {
      ...
      if (!exist) {
        ...
        try {
          newContacts = exist.family_phone ? JSON.parse(exist.family_phone) : [{ name: "Familia", phone: exist.family_phone }];
        } catch(e) {
          newContacts = [];
        }
    ...
    <div className="p-6 flex flex-col h-full bg-white items-center justify-center text-center">
      ...
      {view === "s_code" && (
        <div className="w-full space-y-6">
          ...
          <button onClick={async () => { 
            ...
            if (data) onAccess("senior", code, data.senior_name); 
            else toast.error("PIN no válido"); 
          }} className="w-full bg-[#1EA851] text-white p-5 rounded-2xl text-xl font-black uppercase">Entrar</button>
        </div>
      )}
      ...
      {view === "f_reg" && (
        <div className="w-full space-y-2">
          <input className="w-full p-3 bg-slate-50 border rounded-xl text-sm font-bold" placeholder="Tu Nombre (Ej: Iker)" value={regN} onChange={e => setRegN(e.target.value)} />
          ...
          <input className="w-full p-3 bg-blue-50 border rounded-xl text-sm font-bold" placeholder="PIN de la abuela" value={code} onChange={e => setCode(e.target.value)} />
